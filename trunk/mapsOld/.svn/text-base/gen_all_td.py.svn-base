...
#            print "-----------------------------------------------------------"
    ...
    for i in range(len(addresses)):
        outfile.write("insert into ubid.Houses (markerid, address, latitude, longitude, unit_type, minBeds, maxBeds, units, units_avail, rent, company) VALUES ('" + str(i) + "', '" + addresses[i] + "', '" + str(latitudes[i]) + "', '" + str(longitudes[i]) + "', '" + types[i] + "', '" + str(min_beds[i]) + "', '" + str(max_beds[i]) + "', '" + str(units[i]) + "', '" + str(units_avail[i]) + "', '" + str(rents[i]) + "', 'Campus Management');\n");
